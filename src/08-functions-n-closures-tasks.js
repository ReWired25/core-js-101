 ...
 * retryer() => 2
 */
function retry(funcArg, attemptsArg) {
  const func = funcArg;
...
function logger(func, logFunc) {
  return (...x) => {
    const startStr = `${func.name}(${JSON.stringify(x).slice(1, -1)}) starts`;
    logFunc(startStr);

    const result = func(...x);

    const endStr = `${func.name}(${JSON.stringify(x).slice(1, -1)}) ends`;
    logFunc(endStr);
